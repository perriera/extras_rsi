...
SCENARIO("Mock UploaderInterface: current", "[UploaderInterface]") {
    ...
    rsi::Parameter wrapped_parcel_onClient;
    ...
    When(Method(vendor_server, transfer))
        .AlwaysDo(
            [&_filename_onServer]() {
                rsi::FileNotFoundException::assertion(_filename_onServer, __INFO__);
                std::cout << extras::cyan << extras::pass(" processes file ") << std::endl;
                std::cout << extras::blue << std::endl;
                auto cmd = "ls -la " + _filename_onServer;
    ...
    When(Method(downloader_server, transfer))
        .AlwaysDo(
            [&_filename_onServer, &wrapped_parcel, &original]() {
                rsi::FileNotFoundException::assertion(_filename_onServer, __INFO__);
                rsi::ParcelImploder parcelImploder;
                auto wrapped = parcelImploder.wrap(_filename_onServer);
                rsi::FileNotFoundException::assertion(wrapped, __INFO__);
                wrapped_parcel = wrapped;
