...
void exparx::rsi::send_string(const std::string& msg, int sockfd) {
...
void exparx::rsi::send_int(int msg, int sockfd) {
    std::string value = std::to_string(msg);
    return send_string(value, sockfd);
}

void exparx::rsi::send_line(const std::string& msg, int sockfd) {
    std::string line = msg + "\n";
